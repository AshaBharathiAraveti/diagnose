...
def main():
    ...
    data_file = os.path.join(project_root.parent, "diverse_food_dataset_5000.csv")
    if os.path.exists(data_file):
        ...
        try:
            ...
            if 'temperature' in df.columns: df['temperature_c'] = df['temperature']
            if 'humidity' in df.columns: df['humidity_percent'] = df['humidity']
            if 'gas' in df.columns: df['gas_ppm'] = df['gas'] / 500  # Scale down large raw gas values somewhat resembling PPM
            if 'day' not in df.columns: df['day'] = 1  # default
            if 'spoiled' not in df.columns:
                if 'freshness_score' in df.columns:
                    df['spoiled'] = (df['freshness_score'] < 40).astype(int)
                else:
                    df['spoiled'] = (df['remaining_days'] < 1).astype(int)
    ...
    try:
        ...
        backend_model_path = os.path.join(project_root.parent, "backend", "custom_food_model.pkl")
        model.save_model(backend_model_path)
        print(f"\n💾 Model saved to: {model_file} and {backend_model_path}")
